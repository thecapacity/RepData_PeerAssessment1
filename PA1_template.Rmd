...
Within this dataset there are ```r sum( is.na(my_data$steps) )``` missing values (i.e. the total number of rows with ```NA``` values).
...
The ```new_mean``` is ```r new_mean```, which differs from the original value by ```r new_mean - original_mean```.

The ```new_median``` is ```r new_median```, which differs from the original value by ```r new_median - original_median```.
...
Previously, there were ```r sum( is.na(my_data$steps) )``` missing values, whereas the calculation are now performed over the full ```r nrow(new_data)``` observations.
